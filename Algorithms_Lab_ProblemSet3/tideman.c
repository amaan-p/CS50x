...
void sort_pairs(void)
{
    ...
    {
        int max_indx = i;
        for (int j = pair_count - 1; j >= 1; j--)
        {
            if (preferences[pairs[max_indx].winner][pairs[max_indx].loser] < preferences[pairs[j].winner][pairs[j].loser])
            {
                max_indx = j;
            }
        }
        pair temp = pairs[i];
        pairs[i] = pairs[max_indx];
        pairs[max_indx] = temp;
    }
    return;
}
